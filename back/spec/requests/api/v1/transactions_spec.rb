...
RSpec.describe "Api::V1::Transactions", type: :request do
  ...
  describe "GET /api/v1/transactions" do
    before do
      create(:transaction, user: user, amount: 5000, transaction_type: :income, description: "給料", category: "給与", date: Date.current)
      create(:transaction, user: user, amount: 1000, transaction_type: :expense, description: "食費", category: "食費", date: Date.current)
      create(:transaction, user: user, amount: 3000, transaction_type: :income, description: "先月の給料", category: "給与", date: 1.month.ago)
    end
    ...
    it "期間フィルタが機能する" do
      get "/api/v1/transactions", params: { start_date: Date.current.beginning_of_month, end_date: Date.current.end_of_month }, headers: headers
      expect(response).to have_http_status(:ok)
      json = JSON.parse(response.body)
  ...
  describe "POST /api/v1/transactions" do
    let(:valid_params) do
      { transaction: { amount: 2000, transaction_type: 'expense', description: '交通費', category: '交通', date: Date.current } }
    end
  ...
  describe "PATCH /api/v1/transactions/:id" do
    let!(:transaction) { create(:transaction, user: user, amount: 1000, transaction_type: :expense, description: "食費", category: "食費", date: Date.current) }
    ...
    it "他のユーザーの取引にはアクセスできない" do
      ...
      other_transaction = create(:transaction, user: other_user, amount: 500, transaction_type: :expense, description: "他人の取引", category: "その他", date: Date.current)
