class AchievementService
  ...
  def update_savings_achievements(amount)
    # 総貯金額を取得
    current_total = @user.total_savings
  ...
  def update_milestone_achievements(total_savings)
    ...
    milestone_achievements = @user.achievements.where(
      original_achievement_id: ['savings_milestone_1000', 'savings_milestone_5000', 'savings_milestone_10000', 'savings_milestone_30000'],
      unlocked: false
    )
...
# 特別実績を更新
  def update_special_achievements(achievement_type, data = {})
    special_achievements = @user.achievements.where(category: 'special', unlocked: false)
